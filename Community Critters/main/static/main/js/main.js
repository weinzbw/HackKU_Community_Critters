...
function critterBox(elmnt) {
    ...
    if (elmnt.checked) {
        ...
        gameData.cprogress = Math.round(gameData.cprogress - 5);
        progress.value = gameData.cprogress;
...
function updateCritterLvl(direction) {
    ...
    if (direction > 0) {
        ...
        gameData.cprogress = Math.round(gameData.cprogress - gameData.cprog_max);
...
function updateCritterPic() {
    const image = document.getElementById("critter_img");
    if (gameData.clevel < 10) {
        image.src = critterImages.baby;
        image.alt = "Baby Kitten";
    } else if (gameData.clevel < 20) {
        image.src = critterImages.adult;
        image.alt = "Adult Kitten";
    } else {
        image.src = critterImages.godly;
...
function updateWorldLvl(direction) {
    ...
    if (direction > 0) {
        ...
        gameData.wprogress = Math.round(gameData.wprogress - gameData.wprog_max);  // Subtract the current progress before increasing max
...
function updateWorldPic() {
    const image = document.getElementById("world_img");
    if (gameData.wlevel < 1) {
        image.src = worldImages.dirty;
        image.alt = "Dirty World";
    } else if (gameData.wlevel < 2) {
        image.src = worldImages.nice;
        image.alt = "Nice World";
    } else {
        image.src = worldImages.perfect;
